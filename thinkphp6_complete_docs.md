...
>[danger] 手册阅读须知：本手册仅针对ThinkPHP`6.*`版本。

## 框架简介
![](./assets/external/box_kancloud_cn__2015-12-12_566b6a10506a4.png)

### 🤖 [官方AI开发智能体](<https://chat.topthink.com/chat/eorole>) —— 基于官方手册的数据训练并提供精准解答服务

ThinkPHP是一个免费开源的，快速、简单的面向对象的**轻量级PHP开发框架**，是为了敏捷WEB应用开发和简化企业应用开发而诞生的。ThinkPHP从诞生以来一直秉承简洁实用的设计原则，在保持出色的性能和至简代码的同时，更注重易用性。遵循`Apache2`开源许可协议发布，意味着你可以免费使用ThinkPHP，甚至允许把你基于ThinkPHP开发的应用开源或商业产品发布/销售。

>[danger] ThinkPHP`6.0`基于精简核心和统一用法两大原则在`5.1`的基础上对底层架构做了进一步的优化改进，并更加规范化。由于引入了一些新特性，ThinkPHP`6.0`运行环境要求`PHP7.2+`，不支持`5.1`的无缝升级（官方给出了[升级指导](1037654)用于项目的升级参考）。

`V6.0`版本由[亿速云](https://www.yisu.com)独家赞助发布
[![](./assets/images/partner_ys.png)](https://www.yisu.com)
...
>[danger] 关于`6.0`的入门建议、代码规范、踩坑指南以及常见问题可以参考官方的《[6.0入门必读](https://www.kancloud.cn/thinkphp/thinkphp6-quickstart/)》教程，推荐新手阅读。

> 现在开始，你可以使用官方提供的[`ThinkChat`](https://chat.topthink.com/)，让你在学习`ThinkPHP`的旅途中享受私人AI助理服务！

[![](./assets/external/support_topthink_com__lfs_99d3651702d149ac198f29372116f9c3434a00b9f636247d6ce85f7dc6e07fc0.dat)](https://chat.topthink.com/)

## 相关资源

* [ThinkPHP轻社区](<https://q.thinkphp.cn/>)
* [ThinkPHP知识库](<https://support.topthink.com/@thinkphp/>)
...
## 官方服务
![](./assets/external/doc_topthink_com__%40knowledge_images_LOGO%E9%A1%B6%E6%83%B3%E4%BA%91.svg)
...
* **ThinkChat**：个人助理服务，支持GPT-4的智能会话服务。[立刻体验](https://chat.topthink.com/)
* **ThinkAI**：统一AI接口服务，一站式轻松地调用各类`LLM`模型和AI能力。[查看更多](https://doc.topthink.com/think-ai/default.html)
* **ThinkBot**：快速构建企业AI应用，灵活对接企业业务系统，无需任何开发即可轻松部署上线，无缝嵌入企业业务系统和网站，同时提供私有化部署方案。[查看更多](https://doc.topthink.com/think-bot/default.html)
...
如果有兴趣再发行或再版本手册的全部或部分内容，不论修改过与否，或者有任何问题，请联系版权所有者 [thinkphp@qq.com](mailto:thinkphp@qq.com)。 
对ThinkPHP有任何疑问或者建议，请进入官方讨论区 \[ [http://www.thinkphp.cn/topic](http://www.thinkphp.cn/topic) \] 发布相关讨论。

有关ThinkPHP项目及本文档的最新资料，请及时访问ThinkPHP项目主站 [http://www.thinkphp.cn](https://www.thinkphp.cn/)。
...
>[info] 响应（`Response`）对象用于动态响应客户端请求，控制发送给用户的信息。通常用于输出数据给客户端或者浏览器。

`ThinkPHP`的`Response`响应对象由`think\Response`类或者子类完成，ThinkPHP的响应输出是自动的（命令行模式除外），最终会调用`Response`对象的`send`方法完成输出。

>[danger] `Response`类不能直接实例化，必须使用 `Response::make()` 静态方式创建，建议直接使用系统提供的助手函数完成。
...
>[info] 除了`redirect`函数的默认返回状态码是`302`之外，其它方法没有指定状态码都是返回`200`状态码。
...
>[danger] 这些助手函数的返回值都是`Response`类或者子类的对象实例，所以后续可以调用`Response`基类或者当前子类的相关方法，后面我们会讲解相关方法。
...
>[danger] 下载文件的路径是服务器路径而不是URL路径，如果要下载的文件不存在，系统会抛出异常。
...
# File: 基础/安装ThinkPHP.md

ThinkPHP`6.0`的环境要求如下：

>[info] * PHP >= 7.2.5

>[danger] `6.0`版本开始，必须通过`Composer`方式安装和更新，所以你无法通过`Git`下载安装。
...
>[danger] 更新操作会删除`thinkphp`目录重新下载安装新版本，但不会影响`app`目录，因此不要在核心框架目录添加任何应用代码和类库。

>[info] 安装和更新命令所在的目录是不同的，更新必须在你的应用根目录下面执行
...
>[danger] 实际部署中，应该是绑定域名访问到`public`目录，确保其它目录不在WEB目录下面。


# File: 基础/开发规范.md

## 命名规范
>[danger] 请理解并尽量遵循以下命名规范，可以减少在开发过程中出现不必要的错误。
...
>[danger] 请避免使用PHP保留字（保留字列表参见 [http://php.net/manual/zh/reserved.keywords.php](http://php.net/manual/zh/reserved.keywords.php) ）作为常量、类名和方法名，以及命名空间的命名，否则会造成系统错误。


# File: 基础/目录结构.md

>[info] 相对于`5.1`来说，`6.0`版本目录结构的主要变化是核心框架纳入`vendor`目录，然后原来的`application`目录变成`app`目录。
...
如果你需要一个多应用的项目架构，目录结构可以参考下面的结构进行调整（关于配置文件的详细结构参考后面章节），但首先需要安装ThinkPHP的多应用扩展，具体可以参考[多应用模式](%E5%A4%9A%E5%BA%94%E7%94%A8%E6%A8%A1%E5%BC%8F.md)。
...
>[danger] 多应用模式部署后，记得删除`app`目录下的`controller`目录（系统根据该目录作为判断是否单应用的依据）。

在实际的部署中，请确保只有`public`目录可以对外访问。

>[danger] 在`mac`或者`linux`环境下面，注意需要设置`runtime`目录权限为777。
...
>[danger] `provider.php`服务提供定义文件只能全局定义，不支持在应用下单独定义
...
>[danger] 如果是多应用模式的话配置文件可能同时存在全局和应用配置文件两个同名文件
...
>[danger] 安装了`yaconf`扩展之后，项目里面的配置文件不再有效。而且不再区分全局和应用配置。
...
![](../assets/external/img_kancloud_cn__52_18_52181406361448d118fe78f5777199e9_932x242.png)  
...
要开启一键协程，需要配置如下参数
![](../assets/external/img_kancloud_cn__66_c5_66c57507adea3a39dbe4495abd2d2e29_546x168.png)  

这里需要设置为true，默认已经打开，flags默认即可。  

连接池的配置参数如下：

![](../assets/external/img_kancloud_cn__5c_93_5c93c3a94b9c5f3da44b4cb9563b3e8c_512x504.png)  
...
![](../assets/external/box_kancloud_cn__dcbf30b119dc2bb7ec6f41d943b5646c_250x62.png)
...
>[danger] 要使用Db类必须使用门面方式（`think\facade\Db`）调用
...
>[danger] `withAttr`方法可以多次调用，对多个字段定义获取器。
...
>[danger] 使用事务处理的话，需要数据库引擎支持事务处理。比如 `MySQL` 的 `MyISAM` 不支持事务处理，需要使用 `InnoDB` 引擎。
...
>[danger] 注意在事务操作的时候，确保你的数据库连接使用的是同一个。
...
>[info] 启用分布式数据库后，`hostname`参数是关键，`hostname`的个数决定了分布式数据库的数量，默认情况下第一个地址就是主服务器。
...
>[danger] 主从数据库的数据同步工作不在框架实现，需要数据库考虑自身的同步或者复制机制。如果在大数据量或者特殊的情况下写入数据后可能会存在同步延迟的情况，可以调用`master()`方法进行主库查询操作。

>[info] 在实际生产环境中，很多云主机的数据库分布式实现机制和本地开发会有所区别，但通常会采下面用两种方式：
>
...
>[danger] 无论存储过程内部做了什么操作，每次存储过程调用仅仅被当成一次查询。
...
>[danger] 在模型中进行数据集查询，全部返回数据集对象，但使用的是`think\model\Collection`类（继承`think\Collection`），但用法是一致的。
...
>[danger] 对于`UNION`查询以及一些特殊的复杂查询，推荐使用这种方式首先单独查询总记录数，然后再传入分页方法
...
>[danger] 如果需要在分页的时候传入查询条件，可以使用`query`参数拼接额外的查询参数
...
>[danger] 分页查询的排序字段一定要使用索引字段，并且是连续的整型，否则会有数据遗漏。
...
>[danger] 一般情况下，业务数据不建议真实删除数据，系统提供了软删除机制（模型中使用软删除更为方便）。
...
# File: 数据库/查询构造器/原生SQL.md

`Db`类支持原生`SQL`查询操作，主要包括下面两个方法：

>[danger] `V6.0.3+`版本开始，原生查询仅支持Db类操作，不支持在模型中调用原生查询方法（包括`query`和`execute`方法）。
...
>[danger] 注意不支持对表名使用参数绑定


# File: 数据库/查询构造器/时间查询.md

## 时间比较

>[info] 框架内置了常用的时间查询方法，并且可以自动识别时间字段的类型，所以无论采用什么类型的时间字段，都可以统一使用本章的时间查询用法。
...
>[danger] 如果`update`方法和`data`方法同时传入更新数据，则以`update`方法为准。
...
>[danger] 如果没有任何的查询条件 并且也没有调用`order`方法的话 ，`find`查询不会返回任何结果。
...
>[info] `insert` 方法添加数据成功返回添加成功的条数，通常情况返回 1
...
>[info] `insertGetId` 方法添加数据成功返回添加数据的自增主键
...
>[info] `insertAll `方法添加数据成功返回添加成功的条数
...
# File: 数据库/查询构造器/获取查询参数.md

#  获取数据库查询参数

有时候我们的查询条件是不固定的,这时候我们可以通过`getOptions()`获取最终的查询条件.
...
>[danger] 所有的连贯操作都返回当前的模型实例对象（this），其中带\*标识的表示支持多次调用。


# File: 数据库/查询构造器/链式操作/alias.md
...
> always 参数  think-orm v2.0.53时新增

`cache`方法新增`always`参数,开启之后会缓存空数据.
...
# File: 数据库/查询构造器/链式操作/cacheAlways.md
...
# File: 数据库/查询构造器/链式操作/comment.md
...
# File: 数据库/查询构造器/链式操作/distinct.md

DISTINCT 方法用于返回唯一不同的值 。

例如数据库表中有以下数据

![](../../../assets/images/screenshot_1499569390419.png)
...
# File: 数据库/查询构造器/链式操作/duplicate.md
...
# File: 数据库/查询构造器/链式操作/failException.md
...
# File: 数据库/查询构造器/链式操作/fetchSql.md
...
# File: 数据库/查询构造器/链式操作/limit.md

`limit`方法主要用于指定查询和操作的数量。

>[danger] `limit`方法可以兼容所有的数据库驱动类的
...
>[danger] 如果没有指定`desc`或者`asc`排序规则的话，默认为`asc`。
...
# File: 数据库/查询构造器/链式操作/partition.md
...
# File: 数据库/查询构造器/链式操作/procedure.md
...
# File: 数据库/查询构造器/链式操作/replace.md
...
# File: 数据库/查询构造器/链式操作/sequence.md
...
>[danger] UNION 内部的 SELECT 语句必须拥有相同数量的列。列也必须拥有相似的数据类型。同时，每条 SELECT 语句中的列的顺序必须相同。


# File: 数据库/查询构造器/链式操作/where.md
...
>[info] 和`where`方法相同用法的方法还包括`whereOr`、`whereIn`等一系列快捷查询方法，下面仅以`where`为例说明用法。

## 表达式查询

>[danger] 表达式查询是官方推荐使用的查询方式
...
>[danger] 注意使用字符串查询条件和表达式查询的一个区别在于，不会对查询字段进行避免关键词冲突处理。
...
>[danger] 注意，相同的字段的多次查询条件可能会合并，如果希望某一个`where`方法里面的条件单独处理，可以使用下面的方式，避免被其它条件影响。
...
>[danger] 如果是使用`pgsql`数据库驱动的话，请先导入 `/db/connector/pgsql.sql`文件（如果是12以上版本，则使用 `/db/connector/pgsql12.sql`文件）到数据库执行。
...
>[danger] 新版本不支持操作原生`$_SESSION`数组和所有`session_`开头的函数，只能通过`Session`类（或者助手函数）来操作。会话数据统一在当前请求结束的时候统一写入 所以不要在`session`写入操作之后执行`exit`等中断操作,否则会导致`Session`数据写入失败。
...
>[danger] 注意，`Session`写入数据的操作会在请求结束的时候统一进行本地化存储，所以不要在写入`Session`数据之后使用exit等中断操作，可能会导致`Session`没有正常写入。
...
>[danger] 多应用情况下默认`Session`是跨应用的，也就是说多应用之间是共享会话数据的，如果不希望共享会话数据，可以给每个应用设置不同的前缀`prefix`。
...
如果你使用`ThinkPHP 6.1+`版本，并且需要使用`think-filesystem`上传扩展，请额外执行下面安装命令后按本手册使用。

```
composer require topthink/think-filesystem
```

>[danger] 内置的上传只是上传到本地服务器，上传到远程或者第三方平台的话需要安装额外的扩展。
...
> 语言包文件支持多种格式，包括php数组、`yaml`格式及`json`格式（`V6.0.4+`）。
...
>[danger] 如果是自定义驱动，`type`的值则为自定义驱动的类名（包含命名空间）
...
>[danger] 普通模式的URL访问不再支持，但参数可以支持普通方式传值
...
>[danger] 新版部分核心功能使用中间件处理，你可以灵活关闭。包括Session功能、请求缓存和多语言功能。
...
>[danger] 注意，在`end`方法里面不能有任何的响应输出。因为回调触发的时候请求响应输出已经完成了。
...
>[danger] 事件系统的所有操作都通过`think\facade\Event`类进行静态调用

>[danger] `V6.0.3+`版本开始，事件机制不能关闭
...
>[danger] ThinkPHP的事件系统不依赖事件类，如果没有额外的需求，仅通过事件标识也可以使用，省去定义事件类的麻烦。
...
>[danger] `AppInit`事件定义必须在全局事件定义文件中定义，其它事件支持在应用的事件定义文件中定义。
...
>[danger] 如果你没有特殊的自定义需求，无需对入口文件做任何的更改。

>[info] 入口文件位置的设计是为了让应用部署更安全，请尽量遵循`public`目录为唯一的`web`可访问目录，其他的文件都可以放到非WEB访问目录下面。
...
>[danger] 如果要使用多应用模式，你需要安装多应用模式扩展`think-multi-app`。
...
>[danger] 自动多应用模式下，路由是每个应用独立的，所以你没法省略URL里面的应用参数。但可以使用域名绑定解决。
...
>[danger] 注意：应用支持使用`composer`包，这个时候目录可能是`composer`包的类库所在目录。
...
>[danger] 多应用模式并非核心内置模式，官方提供的多应用扩展更多是抛砖引玉，你完全可以通过中间件来扩展适合自己的多应用模式
...
>[danger] 容器类的工作由`think\Container`类完成，但大多数情况我们只需要通过`app`助手函数或者`think\App`类即可容器操作，如果在服务类中可以直接调用`this->app`进行容器操作。
...
>[danger] 绑定标识调用的时候区分大小写，系统已经内置绑定了核心常用类库，无需重复绑定
...
>[danger] 事实上，控制器是可以被跳过的，通过路由我们可以直接把请求调度到某个模型或者其他的类进行处理。
...
>[danger] 模型类并不一定要访问数据库，而且在ThinkPHP的架构设计中，只有进行实际的数据库查询操作的时候，才会进行数据库的连接，是真正的惰性连接。
...
>[danger] `6.0`版本的驱动采用`Composer`的方式安装和管理。
...
>[info] 如果`before_write`、`before_insert`、 `before_update` 、`before_delete`事件方法中返回`false`或者抛出`think\exception\ModelEventException`异常的话，则不会继续执行后续的操作。
...
>[danger] 修改器方法仅对模型的写入方法有效，调用数据库的写入方法写入无效，例如下面的方式修改器无效。
~~~php
$user = new User();
$data['name'] = 'THINKPHP';
$data['email'] = 'thinkphp@qq.com';
...
>[danger] 避免在模型内部使用复杂的`join`查询和视图查询。
...
>[danger] 一个模型可以定义多个不同的关联，增加不同的关联方法即可
...
>[danger] 推荐关联属性统一使用后者，和数据表的字段命名规范一致，因此在很多时候系统自动获取关联属性的时候采用的也是后者。
...
>[danger] 模型关联支持调用模型的方法

>[info] 具体不同的关联关系的详细使用，请继续参考后面的内容。
...
>[info] 请确保你已经在数据库配置文件中配置了数据库连接信息，如不清楚请参考数据库一章
...
>[danger] 模型自动对应的数据表名称都是遵循小写+下划线规范，如果你的表名有大写的情况，必须通过设置模型的`table`属性。
...
>[danger] `connection`属性使用用配置参数名（需要在数据库配置文件中的`connections`参数中添加对应标识）。
...
>[danger] 模型不支持对数据表的前缀单独设置，并且也不推荐使用数据表的前缀设计，应该用不同的库区分。当你的数据表没有前缀的时候，`name`和`table`属性的定义是没有区别的，定义任何一个即可。
...
>[danger] `init`必须是静态方法，并且只在第一次实例化的时候执行，并且只会执行一次
...
>[danger] 你在模型里面定义的`name`和`table`属性无需包含后缀定义
...
>[danger] 搜索器通常会和查询范围进行比较，搜索器无论定义了多少，只需要一次调用，查询范围如果需要组合查询的时候就需要多次调用。
...
>[danger] `save`方法新增数据返回的是写入的记录数（通常是`1`），而不是自增主键值。
...
>[danger] 不要在同一个实例里面多次新增数据，如果确实需要多次新增，可以使用后面的静态方法处理。
...
>[info] 新增数据的最佳实践原则：使用`create`方法新增数据，使用`saveAll`批量新增数据。
>
...
>[danger] 时间字段的自动写入仅针对模型的写入方法，如果使用数据库的更新或者写入方法则无效。

>[danger] 时间字段输出的时候会自动进行格式转换，如果不希望自动格式化输出，可以把数据库配置文件的 `datetime_format` 参数值改为`false`
...
>[danger] 模型查询除了使用自身的查询方法外，一样可以使用数据库的查询构造器，返回的都是模型对象实例。但如果直接调用查询对象的方法，IDE可能无法完成自动提示。
...
>[danger] 如果你是在模型内部获取数据，请不要使用`$this->name`的方式来获取数据，请使用`$this->getAttr('name')` 替代。
...
>[danger] `value`和`column`方法返回的不再是一个模型对象实例，而是纯粹的值或者某个列的数组。
...
>[info] 模型查询的最佳实践原则是：在模型外部使用静态方法进行查询，内部使用动态方法查询，包括使用数据库的查询构造器。


# File: 模型/模型关联/一对一关联.md
...
>[danger] 有一点需要注意的是，关联方法的命名规范是驼峰法，而关联属性则一般是小写+下划线的方式，系统在获取的时候会自动转换对应，读取`user_profile`关联属性则对应的关联方法应该是`userProfile`。
...
~~~
>[info] 你必须给User模型定义一个名称是`cards`的关联方法。
...
>[danger] `with`方法只能调用一次，请不要多次调用，如果需要对多个关联模型预载入使用数组即可。
...
>[danger] 注意这里的类型约束必须使用 `think\model\Relation`，因为`withLimit`方法是关联类才有的方法
...
>[info] ### belongsToMany('关联模型','中间表','外键','关联键');
...
>[danger] `attach`方法的返回值是一个`Pivot`对象实例，如果是附加多个关联数据，则返回`Pivot`对象实例的数组。
...
>[info] ### hasOneThrough('关联模型', '中间模型', '外键', '中间表关联键','当前模型主键','中间模型主键');
...
>[danger] 远程一对多关联，需要同时存在`Topic`和`User`模型，当前模型和中间模型的关联关系可以是一对一或者一对多。

`hasManyThrough`方法的参数如下：

>[info] ### hasManyThrough('关联模型', '中间模型', '外键', '中间表关联键','当前模型主键','中间模型主键');
...
>[danger] 如果同时还在模型里面定义了相同字段的获取器，则动态获取器优先，也就是可以临时覆盖定义某个字段的获取器。
...
~~~
>[danger] 注意：对于`MorphTo`关联不支持使用动态获取器。
...
>[danger] 注意：`filter`方法的数据处理和获取器并不冲突，`filter`处理的数据会改变模型的原始数据，获取器只有在取值或输出的时候才会进行处理。
...
```
>[danger] 另外，注意，虚拟模型不再支持自动时间戳功能，如果需要时间字段需要在实例化的时候传入。
...
>[danger] 要注意模板文件位置是相对于应用的入口文件，而不是模板目录。
...
# File: 请求/Action参数绑定.md

## 参数绑定

参数绑定是把当前请求的变量作为操作方法（也包括架构方法）的参数直接传入，参数绑定并不区分请求类型。

>[danger] 参数绑定传入的值会经过全局过滤，如果你有额外的过滤需求可以在操作方法中单独处理。
...
>[info] `method`方法返回的请求类型始终是大写，这些方法都不需要传入任何参数。

>[danger] 没有必要在控制器中判断请求类型再来执行不同的逻辑，完全可以在路由中进行设置。
...
>[danger] 你可以设置为任何合法的请求类型，包括`GET`、`POST`、`PUT`和`DELETE`等，但伪装变量`_method`只能通过POST请求进行提交。
...
>[danger] 在命令行下面执行的话，请求类型返回的始终是`GET`。
...
>[danger] 如果`cache`参数传入`false`，则表示关闭当前路由的请求缓存（即使开启全局请求缓存）。
...
>[danger] `param`方法会把当前请求类型的参数和路由变量以及GET请求合并，并且路由变量是优先的。
...
>[danger] 除了`server`和`env`方法的变量名不区分大小写（会自动转为大写后获取），其它变量名区分大小写。
...
>[danger] 框架默认没有设置任何全局过滤规则，你可以在`app\Request`对象中设置`filter`全局过滤属性：
...
![](../assets/images/screenshot_1499323303935.png)

## 查看SQL日志

如果开启了数据库的日志监听（`trigger_sql`)的话，可以在日志文件（或者设置的日志输出类型）中看到详细的SQL执行记录。

>[danger] 通常我们建议设置把SQL日志级别写入到单独的日志文件中，具体可以参考日志处理部分。
...
>[danger] `getLastSql`方法只能获取最后执行的`SQL`记录。
...
>[danger] 为了方便查看，某些核心对象由于定义了`__debugInfo`方法，因此在`dump`输出的时候属性可能做了简化。
...
>[danger] 强烈建议在开发阶段始终开启调试模式（直到正式部署后关闭调试模式），方便及时发现隐患问题和分析、解决问题。
...
>[danger] 由于调试模式没有任何缓存，因此涉及到较多的文件IO操作和模板实时编译，所以在开启调试模式的情况下，性能会有一定的下降，但不会影响部署模式的性能。
...
>[danger] 出于安全考虑，`V6.0.3+`版本开始已经取消了调试模式的默认异常页面中的环境变量显示。
...
![](../assets/images/screenshot_1499243322267.png)

LOGO后面的数字就是当前页面的执行时间（单位是秒） 点击该图标后，会展开详细的Trace信息，如图：

![](../assets/images/screenshot_1570076695969.png)
...
![](../assets/images/screenshot_1570076878177.png)
...
![](../assets/images/screenshot_1570077452057.png)
...
>[danger] 如果你开启了路由延迟解析，需要生成路由映射缓存才能支持全部的路由地址的反转解析。
...
>[danger] 系统默认的变量规则设置是`\w+`，只会匹配字母、数字、中文和下划线字符，并不会**匹配特殊符号以及其它字符**，需要定义变量规则或者调整默认变量规则。
...
>[danger] 不需要开头添加`^`或者在最后添加`$`，也不支持模式修饰符，系统会自动添加。
...
>[info] 闭包中可以使用路由的其它方法，包括路由分组，但不能再包含域名路由
...
>[danger] 一旦设置了MISS路由，相当于开启了强制路由模式
...
>[danger] 如果使用了嵌套分组的情况，子分组会继承父分组的参数和变量规则，而最终的路由规则里面定义的参数和变量规则为最优先。
...
>[danger] 通过路由分组或者域名路由来定义路由才能发挥延迟解析的优势。
...
>[danger] `mergeRuleRegex`方法只能用于路由分组或者域名路由（域名路由其实是一个特殊的分组）。
...
>[danger] 路由参数可以在定义路由规则的时候直接传入（批量），推荐使用方法配置更加清晰。
...
>[danger] URL后缀如果是全局统一的话，可以在路由配置文件中设置`url_html_suffix`参数，如果当前访问的URL地址中的URL后缀是允许的伪静态后缀，那么后缀本身是不会被作为参数值传入的。
...
![](../assets/images/screenshot_1499246102390.png)

>[info]只有在调试模式下面才能显示具体的错误信息，如果在部署模式下面，你可能看到的是一个简单的提示文字，例如：

![](../assets/images/screenshot_1573042348341.png)
...
>[info] 日志遵循`PSR-3`规范,除非是实时写入的日志,其它日志都是在当前请求结束的时候统一写入的 所以不要在日志写入之后使用`exit`等中断操作会导致日志写入失败。
...
```
>[danger] 为避免内存溢出，在命令行下面执行的话日志信息会自动实时写入。
...
>[danger] 默认情况下是不会记录HTTP异常日志（避免受一些攻击的影响写入大量日志），除非你接管了系统的异常处理，重写了`report`方法。
...
>[info] 单文件日志也支持`max_files`参数设置，因为单文件日志同样会生成多个日志备份文件而导致日志文件数据过大。
...
# `6.0`升级到`6.1`版本
由于安全性原因，`6.1`版本移除核心对`think-filesystem`库的依赖，因此`6.0`版本升级至`6.1`版本后，需要单独安装`topthink/think-filesystem`库。
...
![](../assets/images/screenshot_1573614776541.png)
...
![](../assets/images/screenshot_1573615141590.png)
...
>[danger] 系统内置的验证错误提示均支持多语言（参考框架目录下的`lang/zh-cn.php`语言定义文件）。
...
>[danger] 验证场景仅针对验证器有效，独立验证不存在验证场景的概念
...
>[danger] 如果使用了闭包进行验证，则不再支持对该字段使用多个验证规则。
...
>[danger] 属性方式定义验证规则不支持使用对象化规则定义和闭包定义
