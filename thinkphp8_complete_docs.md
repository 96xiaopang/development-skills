...
> [danger] 数据库和模型操作采用独立的[ThinkORM](https://github.com/top-think/think-orm)库，默认安装应用的时候会自动安装，如果你不需要使用该`ORM`库的话，可以单独卸载`topthink/think-orm`后安装其它的`ORM`库。
...
>[danger] 本手册内容主要针对`ThinkORM3.0`，如果你使用的是`ThinkORM4+`，请参考[ThinkORM手册](<https://doc.thinkphp.cn/@think-orm/v4_0/default.html>)，将更具有参考价值。
^
相对于`Db`类来说模型的优势主要在于：
...
![](./assets/topwrite/assets/%E6%9C%AA%E5%91%BD%E5%90%8D%E7%9A%84%E8%AE%BE%E8%AE%A1_%2811%29.png)
...
> [danger] ThinkPHP`8.x`基于PHP`8.0`对`6.1`版本进行了重构和优化，并更加规范化，后续版本可能会陆续提升PHP的版本要求。
...
## 官方服务
![](https://doc.topthink.com/@knowledge/images/LOGO%E9%A1%B6%E6%83%B3%E4%BA%91.svg =250)
...
# File: 命令行/debug_output_level.md
...
# File: 命令行/php_think_optimize.md
...
# File: 命令行/php_think_optimize_route.md
...
# File: 命令行/think_optimize_config.md
...
# File: 基础/development_specifications.md
...
# File: 基础/directory_structure.md
...
如果你需要一个多应用的项目架构，目录结构可以参考下面的结构进行调整（关于配置文件的详细结构参考后面章节），但首先需要安装ThinkPHP的多应用扩展，具体可以参考[多应用模式](<multi_app_model.md>)。
...
![](../assets/topwrite/assets/image.png)
...
# File: 控制器/controller_middleware.md
...
# File: 控制器/resource_controller.md
...
# File: 数据库/查询构造器/aggregation_query.md
...
# File: 数据库/查询构造器/pagination_query.md
...
# File: 数据库/查询构造器/query_express.md
...
# File: 数据库/查询构造器/query_params.md
...
# File: 数据库/查询构造器/sql_query.md

`Db`类支持原生`SQL`查询操作，主要包括下面两个方法：

>[danger] 原生查询仅支持Db类操作，不支持在模型中调用原生查询方法（包括`query`和`execute`方法）。
...
>[danger] 如果安装了多应用模式扩展，还需要在应用的中间件定义文件中添加上述定义。
...
>[danger] `8.0`版本的驱动采用`Composer`的方式安装和管理。
...
# File: 架构/dependency_injection.md
...
> [danger]请根据实际项目需求选择是否采用多模块还是多应用，通常情况下建议使用多模块。
...
![](../assets/topwrite/assets/image_1738913333978.png)
...
>[danger] 注意：字段映射后获取和设置映射字段的值的时候，字段名必须和映射名保持一致，系统不会自动进行驼峰转换。
...
# File: 模型/model_collections.md

## 数据集

> 模型的`select`查询方法返回数据集对象 `think\model\Collection`，该对象继承自`think\Collection`，因此具有数据库的[数据集类](<collections.md>)的所有方法，而且还提供了额外的模型操作方法。
...
>[danger] 模型不支持对数据表的前缀单独设置，并且也不推荐使用数据表的前缀设计，应该用不同的库区分。当你的数据表没有前缀的时候，`name`和`table`属性如果同时定义则`table`属性优先。
...
# File: 模型/model_event.md

## 模型事件

模型事件是指在进行模型的查询和写入操作的时候触发的操作行为。

>[danger] 模型事件只在调用模型的方法生效，使用Db查询构造器操作是无效的 
...
>[danger] 查询范围只能支持`find`或者`select`查询。
...
# File: 模型/模型关联/belongs_to_many.md
...
>[danger] 注意：如果`hasWhere`需要和`where`同时使用的话，`hasWhere`必须在前面。
...
# File: 模型/模型关联/has_many_through.md
...
# File: 模型/模型关联/has_one_through.md
...
>[danger] 注意：如果`hasWhere`需要和`where`同时使用的话，`hasWhere`必须在前面，否则`where`不会生效。
...
# File: 模型/模型关联/relation_aggregation.md
...
# File: 模型/模型关联/relation_output.md
...
# File: 模型/模型关联/relation_preload.md
...
>[danger] `param`方法会把当前请求类型的参数和GET请求合并。
...
>[danger] 出于安全考虑，调试模式的默认异常页面不会显示环境变量。


# File: 调试/sql_debug.md
...
>[danger] 如果你给域名路由设置了绑定，则跨域路由可能不会生效。
...
可以通过命令行快速创建一个资源控制器类（参考后面的控制器章节的[资源控制器](<resource_controller.md>)一节）。
...
分组路由支持所有的路由参数设置，具体参数的用法请参考[路由参数](<route_options.md>)章节内容。
...
Route::group('api');
```
> [danger] 分组目录下的路由定义文件中的所有路由和定义均针对所在分组，如果需要分组嵌套的话，和上面一样嵌套注册子目录分组即可。
...
> [danger]分组绑定后会自动追加对应的`prefix`，并且一旦对分组进行绑定，当该分组下的路由没有匹配到的话，会自动采用默认规则（相当于注册了一个分组MISS路由）。
...
更多中间件的用法参考架构章节的[中间件](middleware.md)内容。


# File: 路由/route_options.md
...
`pattern`方法仅支持对变量设置正则验证，`V8.1.0+`版本开始可以使用`when`方法替代，指定某个变量的多个验证规则（支持所有的内置验证类规则，参考[内置验证规则](<rule_buildin.md>)），两种验证方法按需使用即可。
...
# File: 错误和日志/exception_handle.md
...
![](../assets/topwrite/assets/image_1722309775414.png)
...
# File: 验证/validate_group.md

## 验证分组
> [danger]本功能需要`V8.1.2+`版本支持
...
>[danger] 自定义的验证规则方法名不能和已有的规则冲突，并且必须使用驼峰命名规范。
